...
public class SideslipListView extends ListView {
    ...
    private void performActionDown(MotionEvent ev) {
        ...
        if (mPointChild == null) {//当item为空的时候不拦截
            return;
        }
    ...
    private void performActionUp(MotionEvent ev) {
        //如果向左滑出超过隐藏的四分之一就全部显示
        if (mItemLayoutParams == null) {
            return;
        }
        if (-mItemLayoutParams.leftMargin >= mDeleteWidth / 4) {
     ...
     * 转换为正常隐藏情况
     */
    public void turnNourmal() {
        if (mItemLayoutParams == null || mPointChild == null) {
            return;
        }
